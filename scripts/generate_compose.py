...
from hfl_cicids.config import leaS, REGIONS, leas_by_region
...
x-superexec-app: &superexec-app
  image: lea-ids-hfl-superexec:${HFL_IMAGE_TAG:-latest}
  build:
    context: .
    dockerfile_inline: |
...
def _lea_supernode(lea_id: str, region: str) -> str:
    service = lea_id.replace("_", "-")
  ...
  {service}-supernode:
    ...
    command:
      ...
      - process
      - --node-config
      - 'role=\"lea\" region=\"{region}\" lea-id=\"{lea_id}\" data-dir=\"/data\"'
    depends_on:
  ...
  {service}-superexec-clientapp:
    ...
    volumes:
      - ./data/partitions/{lea_id}:/data:ro
      - ./shared:/shared
    depends_on:
      - {service}-supernode
"""
...
def _flat_supernodes() -> str:
    chunks = []
    for lea in leaS:
        service = f"flat-{lea.lea_id.replace('_', '-')}"
        chunks.append(
            f"""\
  {service}-supernode:
    image: flwr/supernode:${{FLWR_VERSION:-1.29.0}}
    command:
      ...
      - process
      - --node-config
      - 'role=\"lea\" region=\"flat\" lea-id=\"{lea.lea_id}\" data-dir=\"/data\"'
    depends_on:
      - flat-superlink
  ...
  {service}-superexec-clientapp:
    ...
    command:
      ...
      - {service}-supernode:9094
    volumes:
      - ./data/partitions/{lea.lea_id}:/data:ro
      - ./shared:/shared
    depends_on:
      - {service}-supernode
"""
...
def render_compose(include_flat: bool = True) -> str:
    ...
    for region in REGIONS:
        ...
        for lea in leas_by_region(region):
            chunks.append(_lea_supernode(lea.lea_id, lea.region))
