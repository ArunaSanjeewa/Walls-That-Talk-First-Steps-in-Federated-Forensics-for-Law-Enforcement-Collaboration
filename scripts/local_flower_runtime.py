...
from hfl_cicids.config import REGIONS, leas_by_region
...
class LocalFlowerRuntime:
    ...
    def summary_lines(self) -> list[str]:
        ...
        for region in REGIONS:
            ...
            for lea in leas_by_region(region):
                port = lea_CLIENTAPP_PORTS[lea.lea_id]
                lines.append(
                    f"  - {lea.lea_id} SuperNode -> {profile} fleet, "
    ...
    def _start_lea_nodes(self) -> None:
        for region in REGIONS:
            fleet_port = SUPERLINK_PORTS[region].fleet
            for lea in leas_by_region(region):
                port = lea_CLIENTAPP_PORTS[lea.lea_id]
                data_dir = self.root / "data" / "partitions" / lea.lea_id
                service = lea.lea_id.replace("_", "-")
                self._launch(
                    f"{service}-supernode",
                    [
                        "flower-supernode",
                        ...
                        "--node-config",
                        (
                            f'role="lea" region="{region}" '
                            f'lea-id="{lea.lea_id}" data-dir="{data_dir}"'
                        ),
                    ],
                )
                _wait_for_port(port)
