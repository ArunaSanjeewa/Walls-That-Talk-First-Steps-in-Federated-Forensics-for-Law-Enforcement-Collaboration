...
from hfl_cicids.config import DEFAULT_PARTITIONS_DIR, leaS
...
def _evaluate_checkpoint(
    ...
    first_dir = partitions_dir / leaS[0].lea_id
    ...
    rows = []
    for lea in leaS:
        data_dir = partitions_dir / lea.lea_id
...
def main(
    ...
    local_checkpoints_dir: Path | None = typer.Option(
        ...
        help="Optional directory with <lea_id>.pt local-only checkpoints.",
    ),
) -> None:
    ...
    if local_checkpoints_dir is not None:
        for lea in leaS:
            local_checkpoint = local_checkpoints_dir / f"{lea.lea_id}.pt"
            local_rows = _evaluate_checkpoint(
                ...
                split=split,
                run_name="local_only",
                model_kind=lea.lea_id,
            )
            rows.extend([row for row in local_rows if row["lea_id"] == lea.lea_id])
