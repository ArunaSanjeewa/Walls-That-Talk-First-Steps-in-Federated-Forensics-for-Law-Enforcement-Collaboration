    ...
    return notebook(
        [
            md(
                ...
                The dataset is **network-flow telemetry**, not medical or clinical data. The lea
                naming convention represents LEA organizations operating local networks. It does
            ...
            md(
                """
                ## 2. Non-IID LEA-network partitions

                `scripts/make_partitions.py` creates six simulated LEA-network sites with seed
                `123`. The site names remain `lea_*` because the demo is about lea network
            ...
            code(
                ...
                if partition_df.empty:
                    ...
                    ax.set_title("Training network-flow rows per simulated LEA site")
                    ax.set_ylabel("rows")
                    ax.set_xlabel("")
    ...
    return notebook(
        [
            ...
            code(
                ...
                if detail_path.exists():
                    plot_df = detail.set_index("lea_id")[["eval_f1", "eval_roc_auc", "eval_auprc"]]
                    ax = plot_df.plot(kind="bar", figsize=(10, 4), color=["#0f766e", "#2563eb", "#9333ea"])
                    ax.set_title("Global checkpoint metrics by LEA-network site")
                    ax.set_ylim(0, 1.05)
                    ax.set_xlabel("")
                    ax.set_ylabel("score")
                    plt.tight_layout()
    ...
    return notebook(
        [
            md(
                ...
                The flat baseline is a comparison point: all six LEA-network sites connect to
    ...
    return notebook(
        [
            ...
            code(
                ...
                predictions = Path("reports/predictions_lea_eu_01.csv")
