...
def _allocation_weights(
    ...
    base = rng.dirichlet(np.full(len(leas), alpha))
    preference = np.array(
        [3.0 if group in lea.preferred_attack_groups else 0.25 for lea in leas],
        dtype=float,
    )
    if group != "BENIGN":
        preference += np.array(
            [0.50 if lea.lea_id == "lea_na_03" else 0.0 for lea in leas]
        )
    weights = base * preference
    if weights.sum() <= 0:
        weights = np.ones(len(leas), dtype=float)
...
def _repair_min_group_rows(
    ...
    lea_ids = [lea.lea_id for lea in leaS]
    eligible_idx = assignments.index[group_mask]
    if len(eligible_idx) < min_rows * len(lea_ids):
        return

    for lea_id in lea_ids:
        current = assignments.index[(assignments == lea_id) & group_mask]
        deficit = min_rows - len(current)
        if deficit <= 0:
            continue

        donors = []
        for donor_id in lea_ids:
            if donor_id == lea_id:
                continue
        ...
        assignments.loc[moved] = lea_id


def allocate_leas(df: pd.DataFrame, alpha: float, seed: int) -> pd.Series:
    rng = np.random.default_rng(seed)
    lea_ids = np.array([lea.lea_id for lea in leaS])
    ...
    for group, group_index in groups.groupby(groups).groups.items():
        ...
        weights = _allocation_weights(str(group), leaS, rng, alpha)
        assignments.loc[idx] = rng.choice(lea_ids, size=len(idx), p=weights)
...
def _fit_scaler(train_frames: list[pd.DataFrame], feature_cols: list[str]) -> dict:
    ...
    return {
        ...
        "fitted_on": "concatenated simulated LEA-site train splits",
    }
...
def main(
    ...
    output_dir: Path = typer.Option(
        ...
        "--output-dir",
        "-o",
        help="LEA-site partition output directory.",
    ),
    ...
    """Create six non-IID LEA-site partitions and train-only scaler metadata."""
    ...
    assignments = allocate_leas(df, alpha=alpha, seed=seed)
    df = df.assign(lea_id=assignments)

    splits: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]] = {}
    train_frames: list[pd.DataFrame] = []
    for lea in leaS:
        lea_df = df[df["lea_id"] == lea.lea_id].drop(columns=["lea_id"])
        train_df, val_df, test_df = split_lea(
            lea_df,
            val_size=val_size,
            test_size=test_size,
            seed=seed,
        )
        splits[lea.lea_id] = (train_df, val_df, test_df)
    ...
    for lea in leaS:
        train_df, val_df, test_df = splits[lea.lea_id]
        lea_dir = output_dir / lea.lea_id
        lea_dir.mkdir(parents=True, exist_ok=True)

        _transform_split(train_df, scaler).to_parquet(lea_dir / "train.parquet", index=False)
        _transform_split(val_df, scaler).to_parquet(lea_dir / "val.parquet", index=False)
        _transform_split(test_df, scaler).to_parquet(lea_dir / "test.parquet", index=False)

        meta = _metadata(lea, train_df, val_df, test_df, feature_cols)
        (lea_dir / "metadata.json").write_text(json.dumps(meta, indent=2, sort_keys=True))
        console.print(
            f"{lea.lea_id}: train={meta['num_train']:,} "
