...
REPO_URL = "https://github.com/ArunaSanjeewa/Walls-That-Talk-First-Steps-in-Federated-Forensics-for-Law-Enforcement-Collaboration"
...
<svg viewBox="0 0 1060 560" role="img" aria-label="Three-layer hierarchical Flower topology">
  ...
  <text x="55" y="438" class="svg-title">Layer 1: local LEA-site SuperNodes</text>
...
def partition_chart() -> str:
    ...
    for path in metadata_paths:
        ...
        rows.append((meta.get("lea_id", path.parent.name), meta.get("region", ""), benign, attack, total))
    svg_rows = []
    y = 55
    for lea_id, region, benign, attack, total in rows:
        benign_w = int(620 * benign / max_total)
        attack_w = int(620 * attack / max_total)
        svg_rows.append(f'<text x="20" y="{y + 18}" class="chart-label">{esc(lea_id)}</text>')
    ...
    height = y + 35
    return (
        '<figure class="chart"><figcaption>Training network-flow rows per simulated LEA site after non-IID partitioning</figcaption>'
        f'<svg viewBox="0 0 1040 {height}" role="img" aria-label="LEA-site partition chart">'
...
def predictions_table(limit: int = 12) -> str:
    rows = read_csv_rows(REPORTS_DIR / "predictions_lea_eu_01.csv")
    if not rows:
...
<head>
  ...
  <title>{esc(title)} - lea-ids-hfl</title>
  <link rel="stylesheet" href="{rel(prefix, 'styles.css')}">
</head>
<body>
  <header class="topbar">
    <a class="brand" href="{rel(prefix, 'index.html')}">lea-ids-hfl handbook</a>
...
def make_index() -> str:
    ...
    body += callout(
        ...
        "decision variables. In this repository, leas are simulated LEA network operators.",
    )
    body += callout(
        "Fast path",
...
EPISODES: list[Page] = [
    Page(
        ...
        summary="The complete architecture before touching commands: LEA network sites, regions, gateways, and the global hub.",
        body=(
            h2("Demo objective")
            + p(
                "This project simulates LEA organizations collaborating to train an intrusion "
            ...
            + callout(
                "Not clinical data",
                "The lea naming convention is an infrastructure scenario. The dataset is not "
            ...
            + ol(
                [
                    ...
                    "Run regional federations with LEA-network sites as clients.",
            ...
            + p(
                ...
                "layer above it. LEA-network sites expose model updates to a regional SuperLink; a region "
...
"""
    ...
    Page(
        ...
        summary="How CIC-IDS2017 CSVs become six simulated LEA-network site partitions with a non-IID split.",
        body=(
            h2("Cleaning")
            + p(
            ...
            + p(
                "Partitioning assigns attack groups to LEA-network sites with biased weights. "
            ...
            + code(
                """
data/partitions/lea_eu_01/train.parquet
data/partitions/lea_eu_01/val.parquet
data/partitions/lea_eu_01/test.parquet
data/partitions/lea_eu_01/metadata.json
...
...
""",
        ...
        body=(
            ...
            + table(
                ...
                [
                    ...
                    ["SuperNode", "A client connected to a SuperLink. It can be a LEA-network site or a gateway."],
            ...
            + h2("Site SuperNode")
            + p(
                "The site SuperNode represents a LEA organization operating a local network. It connects to its "
            ...
            + notes(
                [
                    ...
                    f"{line_ref('scripts/generate_compose.py', 88, 89, 'Lines 88-89')} pass node config into Flower. The ClientApp later reads <code>role</code>, <code>region</code>, <code>lea-id</code>, and <code>data-dir</code> from this config.",
    ...
    Page(
        ...
        body=(
            ...
            + h2("Site ClientApp")
            + p(
                "When the node role is lea, the service is acting as a LEA-network site. "
    ...
    Page(
        ...
        summary="How the regional server aggregates LEA-network sites and writes regional checkpoints.",
        body=(
            ...
            + p(
                ...
                "The clients are the LEA-network sites connected to the regional SuperLink."
            )
...
docker compose logs --tail=80 lea-eu-01-supernode
...
"""
    ...
    Page(
        ...
        body=(
            ...
            + notes(
                [
                    f"{line_ref('scripts/predict_with_checkpoint.py', 31, 38, 'Lines 31-38')} enforce that both the checkpoint and selected lea split exist.",
        ...
        body=(
            ...
            + table(
                ...
                [
                    ...
                    ["<code>reports/predictions_lea_eu_01.csv</code>", "The trained checkpoint can be used for row-level inference."],
                ],
            )
